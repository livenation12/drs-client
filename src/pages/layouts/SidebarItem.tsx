...
import { SidebarItemProps, SubMenuItemProps } from '@/models/sidebar.model';
...
function HoveredSubMenuItem({ icon, text, active }: SubMenuItemProps) {
  return (
    <div
      className={`my-2 rounded-md p-2 ${active ? 'bg-gray-300' : ' hover:bg-gray-50'
        }`}
    >
    ...
    <>
      <li>
        <button
         ...
         ${active && !subMenu
              ? 'text-gray-50 bg-black dark:bg-gray-700'
              : 'bg:text-gray-50 dark:hover:bg-gray-700 hover:bg-gray-200'
            }
         ${!expanded && 'hidden sm:flex sm:justify-center'}
     `}
